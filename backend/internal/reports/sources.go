...
//
// Всё, что связано со СДВИГОМ платежа, живёт здесь, а не в кодификаторах:
// в БДР статья берёт свой месяц, в БДДС — тот, в котором реально платят.
...
// prevMonth — значение ПРЕДЫДУЩЕГО месяца: платёж сдвинут на месяц
// вперёд. В первом месяце проекта платить нечего — ноль.
//
// Так в БДДС платятся страховые взносы: начислены в августе — уплачены в
// сентябре (audit/service_sheets.md, «в первый месяц ничего, во второй и
// последующие — за предыдущий месяц»).
//
// Обратная сторона правила: начисление последнего месяца проекта в
// горизонт отчёта не попадает — платёж приходится на месяц после его
// окончания. Это не потеря данных, а свойство денежного отчёта:
// в БДР эта сумма стоит на своём месте.
...
// halfAndHalf — платёж двумя частями: половина за текущий месяц и
// половина за предыдущий. В первом месяце проекта — только своя половина,
// второй половине неоткуда взяться.
//
// Так в БДДС платятся зарплата и НДФЛ: два раза в месяц, аванс и расчёт
// (audit/service_sheets.md, «в первый месяц половина, во второй и далее —
// половина от прошлого месяца и половина по этому»).
...
//
// payMonth сопоставляет календарному месяцу платежа тот квартал, за
// который платят: у БДР это свой же квартал, у БДДС — предыдущий.
