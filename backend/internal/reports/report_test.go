...
func TestBDDS_QuarterlyProfitTaxShifted(t *testing.T) {
	...
	// Сдвиг переносит деньги, а не меняет их, но последний квартал в
	// горизонт уже не попадает: I квартал 2028 (янв-мар) БДР начисляет в
	// марте 2028 — последнем месяце проекта, — а БДДС платил бы в апреле,
	// то есть после его окончания. Поэтому итог БДДС ровно на этот
	// квартал меньше. Это свойство денежного отчёта, а не потеря: в БДР
	// сумма стоит на своём месте.
	bdr := Build(KindBDR, res, rf(date(2027, time.January)))
