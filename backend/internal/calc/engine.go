...
func Run(inp *BudgetInputs) *CalcResult {
	...
	case sameExecutor(inp.ExecutorName, ExecutorAibiconKG):
		// База — строго ТКП (2.Бюджет!$G$251). Подстановка расчётной
		// выручки сюда не распространяется: у киргизского филиала ТКП
		// задаётся всегда, а без договора налог считать не от чего.
		//
		// Со справочной ставкой (сейчас 4%) налог берётся от ВСЕГО ТКП
		// один раз: `ТКП × ставка`. Деления на длительность нет — оно
		// было артефактом формы, где та же сумма размазывалась по
		// месяцам. Решение владельца 2026-08-28.
		//
		// Без справочной ставки остаётся формула формы — «5% + 2%» от
		// ТКП, делённого на длительность (два деления на $D$8, по одному
		// в каждом слагаемом, обратного умножения нет). Это запасной путь
		// для версий, сохранённых до появления справочных значений.
