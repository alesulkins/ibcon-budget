...
// defaultRefinancingPct — ставка рефинансирования, % годовых.
...
type InputEmployees struct {
	...
	// PerDiemRF — суточные по РФ. В РАСЧЁТЕ НЕ УЧАСТВУЕТ: 4.6!D9 это формула
	// `=700+300/0.87*1.3`, а не поле ввода, поэтому применяется константа
	// perDiemRFRate.
...
type InputRentApartments struct {
	...
	// CleaningMonths — номера месяцев проекта (1-based), в которых начисляется
	// уборка. Пустой список — уборки нет за весь период.
...
// ItemPurchase — одна разовая покупка: строка таблицы 4.3!A17:D27 (авто) или
// 4.4!A12:D22 (вагончики).
...
// RentedItem — одна строка аренды: авто (4.3 строки 5-6) или гараж (4.3
// строки 10-11).
...
type InputBudgetParams struct {
	...
	// Выручка TargetRentPct — целевая рентабельность БЕЗ налога на прибыль, в
	// процентах (20 означает 20%). Источник: 2.Бюджет!E234, ручной ввод
	// экономиста.
...
type BudgetInputs struct {
	...
	// TransportRental и GarageRent — СТАРЫЙ формат листа 4.3 (готовые суммы по
	// месяцам).
...
type CalcResult struct {
	...
	// (2.Бюджет!249), сумма за первые четыре месяца.
	...
	// OperatingMargin — операционная маржинальность за проект (2.Бюджет!G234),
	// сумма месячных значений строки 234.
