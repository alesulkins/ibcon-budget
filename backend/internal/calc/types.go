...
//
// В форме у таблицы есть колонка «Количество» (4.3!B17:B27), и итог строки
// считается как `количество × стоимость`. В платформе количества нет: две
// одинаковых машины — две строки. Сумма от этого не меняется.
type CarPurchase struct {
	...
	// Стоимость авто (4.3!C17:C27)
	Price float64 `json:"price"`
}

// RentedItem — один арендуемый предмет: авто (4.3 строки 5-6) или гараж
// (4.3 строки 10-11).
//
// Форма хранит это иначе: одна цена на весь вид (4.3!B6 для авто, B11 для
// гаража) и количество по месяцам (4.3!C5:BJ5, C10:BJ10). В платформе
// наоборот — каждый предмет своя строка со своей ценой и своим набором
// месяцев, поэтому разные машины могут стоить по-разному. «Количество в
// месяце» из формы превращается в число строк, включивших этот месяц;
// итоговая сумма совпадает.
type RentedItem struct {
	Name string `json:"name"`
	// Цена аренды за месяц, одна на все месяцы (4.3!B6 / 4.3!B11)
	Price float64 `json:"price"`
