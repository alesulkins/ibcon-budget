...
type Employee struct {
	...
	// MultiplierOverrides — множитель графика (4.6!BU16), заданный руками.
	//
	// Обычно множитель считает формула из графика месяца и оклада
	// (scheduleMultiplier). Экономист может переопределить значение в любом
	// месяце, и тогда в расчёт ФОТ идёт его число, а не вычисленное.
	//
	// Указатели, а не float64: ноль здесь — законное ручное значение
	// («в этом месяце не начисляем»), и его надо отличать от «ячейку не
	// трогали». nil в ячейке И ЕСТЬ флаг «считается автоматически»:
	// отдельного массива признаков не нужно, а рассинхронизировать
	// значение и его флаг попросту нечем.
	//
	// Массив может быть короче графика или отсутствовать вовсе — тогда
	// автоматически считаются все недостающие месяцы.
	MultiplierOverrides []*float64 `json:"multiplier_overrides,omitempty"`
}
...
// (4.3 строки 10-11).
//
// Структура повторяет форму: цена за единицу в месяц плюс количество,
// заданное отдельно в каждом месяце. Единственное расширение — строк может
// быть несколько, у каждой своя цена (в форме цена одна на весь блок:
// 4.3!B6 для авто, 4.3!B11 для гаража). Итог месяца — сумма по всем
// строкам, как 4.3!C5*$B$6 и 4.3!C10*$B$11.
type RentedItem struct {
	...
	// (4.3!C5:BJ5 для авто, 4.3!C10:BJ10 для гаража).
	// 0 — в этом месяце не арендуем, это допустимое значение.
...
type BudgetInputs struct {
	...
	// Лист 4.4 — считается по формуле. Даёт строку 181 (обустройство
...
type CalcResult struct {
	...
	// OperatingMargin — операционная маржинальность за проект
	// (2.Бюджет!G234), сумма месячных значений строки 234.
	//
	// Это наценка на расходы, а не процент: в режиме наценки она равна
	// «расходы × коэффициент», а при заданном ТКП форма её обнуляет —
	// тогда прибыль считается напрямую как выручка минус расходы.
