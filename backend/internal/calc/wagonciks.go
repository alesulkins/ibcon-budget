...
// InputWagonciks — данные листа 4.4. Аренда — цена за единицу плюс
// количество по месяцам (блок 4.4 строки 5-6).
...
func calcWagonciks(in *InputWagonciks, duration int) []float64 {
	...
	for i := range in.Purchases {
		p := &in.Purchases[i]
		// Месяц не заполнен (0) — строка таблицы пустая. Месяц за пределами
		// проекта сюда не доходит: его отклоняет ValidateWagonciks.
...
// ValidateWagonciks проверяет ввод листа 4.4. Месяц покупки должен лежать
// внутри проекта — 1…D, без изъятий: покупка разрешена в любой месяц,
// включая последние.
