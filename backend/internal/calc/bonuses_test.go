...
// Тесты премий и компенсаций при увольнении.
//
// ПРЕМИИ — источник ожидаемых значений: спецификация санкционированного
// отклонения №2 из CLAUDE.md, НЕ Excel. Формула формы дефектна: процент
// в ней захардкожен ('2.Бюджет'!H14*0.5), колонка процентов 4.1!C4:C5
// игнорируется, поэтому сверять премии с формой бессмысленно.
//
// КОМПЕНСАЦИЯ при увольнении — источник: 4.1!BO11 (Excel), формула рабочая,
// сверяется с эталоном из заполненного файла.
...
// TestBonuses_BuilderDay — День строителя 20% в августе от проиндексированного
// оклада. Старт декабрь 2026 → август 2027 это месяц 9, к нему уже прошёл
// апрель 2027 (месяц 5), поэтому база 200 000 × 1.1 = 220 000.
// Источник: спецификация отклонения №2.
...
// TestSeverancePay_ExcelReference — компенсация при увольнении, сверка
// с эталоном из calc_sheets_ibcon-russia.xlsm.
//
// ИСТОЧНИК: 4.1!BO11 (Excel) — эта часть формы рабочая.
//
//	=IF('2.Бюджет'!G14=0; 0;
//	    SUM(H14:BO14)/COUNTIF(...;"<>0")/22*28*COUNTIF(...;"<>0")/12
//	    + 2*'2.Бюджет'!BO14)
//
// Данные сотрудника 1: оклад 200 000, график [4/2, ОФ, МВ, не принят,
// не принят, не принят] → ФОТ по месяцам [200000, 200000, 30000, 0, 0, 0],
// итого 430 000, ФОТ последнего месяца 0.
// Эталон 4.1!BO11 = 45 606.06.
func TestSeverancePay_ExcelReference(t *testing.T) {
	...
	// Второй эталон — сотрудник 3 того же файла: ФОТ по месяцам
	// [100000, 100000, 100000, 100000, 100000, 30000] = 530 000,
	// последний месяц 30 000. Эталон 4.1!BO13 = 116 212.12.
	// Здесь проверяем саму формулу на известных из файла суммах: у этого
	// сотрудника апрель приходится на рабочий месяц, поэтому индексация
	// изменила бы ФОТ и прямое сравнение с Excel стало бы некорректным.
...
// него стоит «не принят». Именно так работает форма: защита смотрит на
// ИТОГ ФОТ за проект (G14), а не на статус последнего месяца.
