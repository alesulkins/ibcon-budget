...
type Estimate struct {
	...
	// Из них — объявления о ДЛИТЕЛЬНОЙ аренде. По ним считаются
	// перцентили: посуточная цена, пересчитанная в месяц, выше месячной
	// в разы и утащила бы 95-й перцентиль вверх. В модель посуточные
	// всё равно идут — там разницу берёт на себя отдельный признак.
	SampleLongTerm int `json:"sample_long_term"`
	...
	// Что закладывать в бюджет. По умолчанию — 95-й перцентиль:
	// в бюджете нужна цена, по которой квартиру точно снимут, а не
	// средняя по рынку.
