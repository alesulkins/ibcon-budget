...
type ExportMeta struct {
	...
	// Employees — сотрудники версии и ITRPositions — названия должностей с
	// признаком ИТР.
	Employees    []calc.Employee
	ITRPositions map[string]bool

	// Params — параметры версии: проценты и сроки банковских гарантий. В
	// CalcResult их нет — там только посчитанные суммы, — а в книге нужно
	// видеть, из чего сумма получилась.
	Params *calc.InputBudgetParams

	// Limited — урезанная выгрузка администратора проекта: строки 178-212 листа
	// «2.Бюджет», то есть накладные расходы по статьям и их итог.
	Limited bool
}
...
// BuildExport собирает книгу с результатами расчёта. reps — листы БДР и
// БДДС, которые лягут в ту же книгу следом за «Бюджетом»: экономисту нужны
// три листа сразу.
func BuildExport(m ExportMeta, r *calc.CalcResult, reps ...*reports.Report) ([]byte, error) {
	...
	// monthLabel — подпись месяца проекта.
	...
	// Вердикт по рентабельности — первым делом, до карточки проекта: книгу
	// открывают, чтобы понять, каков бюджет, а не чей он.
	if !m.Limited {
	...
	// Между блоками — ровно одна пустая строка.
	blank := func() { row++ }
