...
// Кнопка «Добавить …» вынесена в панель вкладок, чтобы таблица начиналась на
// той же высоте, что и в реестре проектов.
...
// Поиск по названию и фильтр по активности — одинаковые во всех справочниках
// (общее требование к справочникам).
function useRefFilter<T extends { active: boolean }>(
  ...
  const filtered = useMemo(() => {
    ...
    // рендер; в зависимости её не берём, иначе фильтр считался бы заново на
    // каждый рендер вкладки.
  }, [rows, search, status]);
...
// сайдбаре — как реестр проектов и история изменений.
