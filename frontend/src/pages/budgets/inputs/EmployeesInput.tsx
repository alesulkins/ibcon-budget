...
export default function EmployeesInput({
  ...
  // Ручной множитель графика на месяц.
    ...
    <div>
          ...
          {PARAM_FIELDS.map(({ key, label, fallback }) => (
            <div key={key}>
              ...
              <InputNumber
                ...
                // умолчанию и поле нельзя было очистить.
      ...
      >
        <Table
          ...
          // Нажатие на строку открывает график: отдельная кнопка «График»
          // занимала место в каждой строке, а нужна она была ровно за тем
          // же.
          onRow={(_, idx) => ({
