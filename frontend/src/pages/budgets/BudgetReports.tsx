...
export default function BudgetReports({ versionId, permissions, readonly }: Props) {
  ...
  // Свёрнутая колонка статей.
  const [foldNames, setFoldNames] = useState(false);
  // Обёртка таблицы и отступ, на котором стоит стрелка сворачивания. Стрелка
  // живёт не в шапке: шапка уезжает вверх при прокрутке, а свернуть колонку
  // нужно в любой момент.
  ...
  return (
    <div>
      ...
      {/**
        ...
        // широком экране: сначала листается страница — уезжают переключатель
        // и пояснение, — а дальше вниз идёт сама таблица.
        */}
      <div ref={setWrap} style={{ position: 'relative' }}>
        ...
        {rows.length > 0 && (
          <div style={{
            ...
            top: '45vh',
            height: 0,
            // Пока страница вверху, липкий элемент стоит на своём месте в
            // потоке — у самого верха таблицы, ровно на шапке.
            marginTop: 48,
            // Выше шапки таблицы (у antd она липкая, z-index 21),
            // закреплённых колонок и полосы прокрутки: иначе круг уходил
            // под них и выглядел обрезанным.
            zIndex: 30,
            pointerEvents: 'none',
          }}>
