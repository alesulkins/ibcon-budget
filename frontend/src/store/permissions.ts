 ...
 *
 * Права считает СЕРВЕР — здесь только копия его ответа, по которой
 * прячутся недоступные кнопки и разделы. Спрятанная кнопка ничего не
 * защищает: тот же запрос можно послать напрямую, и отказ выносит
 * бэкенд (ТЗ 3.2 п.6). Ничего важного на этот модуль не вешаем.
 *
 * Коды прав повторяют auth/permissions.go — список там же.
 */
...
/**
 * Права, которые главный экономист выдаёт индивидуально.
 * «Управление пользователями» в списке нет: выдав его, ГЭ получил бы
 * второго ГЭ — для этого есть смена роли. Бэкенд такой запрос отклонит
 * (auth.IsGrantable), список здесь — чтобы не предлагать заведомо
 * невозможное.
 */
 ...
 * (`UserPanel`, блок «Доступ к проектам»): один список проектов, справа
 * от каждого — одно из этих прав, вместо двух отдельных панелей
 * (тумблер «правка/просмотр» и отдельная выдача индивидуальных прав).
 *
 * «Все права» и «Просмотр проекта» закреплены первыми как самые частые
 * случаи, дальше — остальные индивидуальные права, у которых есть смысл
 * привязки к проекту (внепроектные — создание проекта, справочники —
 * сюда не попадают, их привязать к одному проекту нельзя).
 */
...
/**
 * Главный экономист — единственная роль, у которой все 14 прав действуют
 * на всех проектах (roleMatrix[RoleGE], все клетки ScopeAll). Отдельный
 * доступ к проекту и индивидуальные права ему не нужны и не выдаются:
 * выдавать нечего, всё уже есть от роли. Поэтому в интерфейсе его доступ
 * показывается как факт, а не как редактируемый набор.
 */
