 ...
 *
 * Хранятся в учётной записи (users.ui_settings), а не в браузере, чтобы
 * человек видел свой интерфейс на любом устройстве. Копия лежит и в
 * localStorage — но только чтобы применить настройки ДО ответа сервера:
 * иначе при каждой загрузке страница на секунду мигала бы чужим
 * оформлением.
 */
...
/**
 * Базовый кегль в пикселях для каждого размера.
 *
 * Меняется именно РАЗМЕР ТЕКСТА, а не масштаб страницы: transform:scale
 * растянул бы и рамки с отступами, и вёрстка поехала бы. Здесь же
 * antd пересчитывает свои размеры от базового кегля сам, а наши отступы
 * заданы в px и остаются на месте — сдвигается только текст.
 */
