...
// Тёмная палитра. Не инверсия светлой: цвета подобраны так, чтобы фирменный
// тон остался узнаваемым, а текст не «звенел» на чёрном.
const DARK = {
  text: '#E6EDF0',
  textSoft: '#9FB3BC',
...
// фиксированной палитры: иначе фиолетовый сайдбар стоял бы на синем фоне.
...
// Непрозрачный аналог полупрозрачной подсветки: цвет `over`, наложенный с
// прозрачностью `a` на сплошной `base`. Нужен закреплённым колонкам таблицы.
...
// Читаемый цвет текста на заливке выбранного цвета.
function readableOn(hex: string): string {
  const m = /^#?([0-9a-f]{6})$/i.exec(hex.trim());
  if (!m) return '#fff';
  const n = parseInt(m[1], 16);
  const lum = (0.299 * ((n >> 16) & 255) + 0.587 * ((n >> 8) & 255) + 0.114 * (n & 255)) / 255;
  return lum > 0.55 ? '#16323d' : '#fff';
}
...
export default function ThemedApp() {
  ...
  // Часть оформления живёт в index.css — правилами, которые нельзя задать
  // инлайновым стилем.
  useEffect(() => {
    ...
    // Всплывающее уведомление красится целиком: подложка — выбранный цвет,
    // взятый полупрозрачным, поверх размытия.
    root.setProperty('--ibcon-notice-bg', alpha(noticeColor, 0.7));
    root.setProperty('--ibcon-notice-fg', readableOn(noticeColor));

    // Стекло: подложка панелей, светлая грань и обводка.
    root.setProperty('--ibcon-glass-bg', alpha(white, dark ? 0.42 : 0.48));
    root.setProperty('--ibcon-glass-sheen',
      dark ? 'rgba(255, 255, 255, 0.16)' : 'rgba(255, 255, 255, 0.75)');
    root.setProperty('--ibcon-glass-edge',
      dark ? 'rgba(255, 255, 255, 0.20)' : 'rgba(255, 255, 255, 0.65)');

    // Оттенки фирменного цвета: растяжка сайдбара и шапка входа. Раньше они
    // были вписаны литералами и не менялись вместе с цветом.
  ...
  return (
    <ConfigProvider
      locale={ruRU}
      theme={{
        // cssVar переводит antd на CSS-переменные: стили считаются один раз,
        // а не пересобираются рантайм-движком на каждую новую комбинацию
        // компонентов.
        cssVar: { key: 'ibcon' },
        hashed: false,
