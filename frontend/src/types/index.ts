...
// регистронезависимо: в карточке проекта город записан строкой, а не ссылкой
// на справочник.
...
// Транспорт (лист 4.3). Суммы НЕ передаются: их считает calcTransport на
// бэкенде.
...
// Офис (лист 4.5). Суммы НЕ передаются: их считает calcOffice. Аренда
// (4.5!C5) и уборка (4.5!C11) уходят в ДВЕ разные строки бюджета.
...
// Ввод листа-покупок (4.7 приборы, 4.12 корпоративы). Расход месяца — сумма
// «цена × количество» по строкам этого месяца.
...
export interface InputBudgetParams {
  ...
  // ТКП — стоимость договора (2.Бюджет!G251). Единственный ручной ввод блока
  // выручки.
  contract_value: number;
  // Справочные значения исполнителя, подставленные в форму (проценты).
  // Хранятся в версии бюджета как snapshot: правка справочника не меняет уже
  // сохранённые версии.
