...
// Автосохранение формы шага мастера: дебаунс 1 с после последнего ввода плюс
// интервал раз в 2 минуты. Ручной кнопки «Сохранить» на шагах больше нет.
export function useAutosave<T>({ data, ready, save, enabled = true }: Options<T>) {
  ...
  // Слепок текущего состояния формы. Формы собирают payload заново на каждый
  // рендер, поэтому по ссылке `data` всегда «новый» объект.
